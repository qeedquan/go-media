...
func (r *Rand) Gen1D(x float64) float64 {
	...
	return f64.Lerp(t, r.vals[r.perm[x0]], r.vals[r.perm[x1]])
}
...
func (r *Rand) Gen2D(x, y float64) float64 {
	...
	nx0 := f64.Lerp(sx, c00, c10)
	nx1 := f64.Lerp(sx, c01, c11)

	return f64.Lerp(sy, nx0, nx1)
}

func (r *Rand) Gen2Dv(v f64.Vec2) float64 {
	return r.Gen2D(v.X, v.Y)
}

func Gen1D(x float64) float64    { return defrand.Gen1D(x) }
func Gen2D(x, y float64) float64 { return defrand.Gen2D(x, y) }
func Gen2Dv(v f64.Vec2) float64  { return defrand.Gen2Dv(v) }
