...
func (f *File) calcImageSizes() (rawSizeOfHeaders, sizeOfHeaders, sizeOfImage uint64) {
	...
	sizeOfHeaders = mathutil.Multiple64(sizeOfHeaders, f.FileAlignment)

	sizeOfImage = sizeOfHeaders
	for _, s := range f.Sections {
		sizeOfImage += uint64(mathutil.Multiple64(uint64(s.VirtualSize), f.SectionAlignment))
	}
	sizeOfImage = mathutil.Multiple64(sizeOfImage, f.FileAlignment)
	return
}
...
func (f *File) CreateSection(name string, va, size, flags uint64) (*Section, error) {
	...
	p.Offset = uint32(off)
	p.Size = uint32(size)
	p.VirtualAddress = uint32(mathutil.Multiple64(va, f.SectionAlignment))
	p.VirtualSize = uint32(mathutil.Multiple64(uint64(p.Size), f.SectionAlignment))
	...
	for _, d := range dt {
		for _, y := range d.Symbols {
			if l := uint64(len(y.Name)) + 1; strsize < l {
				strsize = mathutil.Multiple64(l, 128)
			}
			size += strsize
		}
		if thunksize < uint64(len(d.Symbols)+1) {
			thunksize = uint64(len(d.Symbols) + 1)
			thunksize = mathutil.Multiple64(thunksize, 1024)
		}
		size += thunksize
		size += strsize
	}
	if sectsize < size {
		sectsize = mathutil.Multiple64(size, f.SectionAlignment)
	}
